...
function useQueryState(cb: (q: Record<string, string>) => any) {
...
export function SearchBox(props: any) {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    const query = qs.stringify(removeEmpty({
      keywords,
      genre,
  ...
  return (
      <form onSubmit={handleSubmit}>
        <Grid container flexDirection="row-reverse" spacing={2}>
          <Grid item xs={12} sm={4} className={css`position:relative;`}>
            <Input fullWidth
              type="search"
              autoFocus
            name="keywords" placeholder="Search by movie title" value={keywords} onChange={e => setKeywords(e.target.value)}
            ...
            <Grid item xs={12} className={css`padding-top: 0.5rem;text-align: right;`}>
              <Button onClick={props.onClose} color="secondary">cancel</Button>
              {hasFilter && <Button onClick={handleReset} type="reset" color="warning">Reset</Button> }
